...
class Jamk:
    ...
    def valitse_rooli(self):
        ...
        if len(igllä) >= 1 and len(bossit) >= 1 and len(entryt) >= 1 and len(lurkit) >= 1 and len(riflet) >= 1:
            ...
            for AnnaRooli, ListaanPelaaja in self.rooli_pelaajat.items():
                image_path = os.path.join('images', f'{ListaanPelaaja}.png')
                self.window[f'{AnnaRooli.lower()}_image'].update(filename=image_path)

    def näytä_rooli(self):
        roolit_text = ''
        for AnnaRooli, ListaanPelaaja in self.rooli_pelaajat.items():
            roolit_text += f"{AnnaRooli}: {ListaanPelaaja}\n"
    ...
    def run(self):
        while True:
            ...
            # yksi rooli uusiksi
                ...
                AnnaRooli = event.split('_')[1].upper()
                self.rooli_pelaajat[AnnaRooli] = random.choice(self.hae_pelaaja(f'{AnnaRooli.lower()}.txt'))
                self.window[f'{AnnaRooli.lower()}_image'].update(filename=os.path.join('images', f'{self.rooli_pelaajat[AnnaRooli]}.png'))
                self.näytä_rooli()
